...
- Create configurations for the zgda services (located in `inabox/testdata/DATETIME/envs`)
...
To view the configurations created for the ZGDA service components, look in `inabox/testdata/DATETIME/envs`
...
./bin.sh start
```
This will print all logs from the ZGDA services to the screen; `Ctrl+C` will stop all services. Inspect the logs to make sure all binaries started without any errors. 

Alternatively, you can start and stop the ZGDA services in detached mode by running `./bin.sh start-detached` and `./bin.sh stop-detached`, respectively. In this case, the logs are saved to `inabox/testdata/DATETIME/logs`.
